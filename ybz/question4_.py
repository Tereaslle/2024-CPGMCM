...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        # 根据材料类别计算平均误差
        materials = {1: '材料1', 2: '材料2', 3: '材料3', 4: '材料4'}
        errors_dict = {material: [] for material in materials.values()}

        for i in range(len(errors)):
            material = materials.get(x_test_np[i][3], '未知')
            errors_dict[material].append(errors[i])
        ...
        # 创建箱线图
        box = plt.boxplot(
            [errors_dict['材料1'], errors_dict['材料2'], errors_dict['材料3'], errors_dict['材料4']],
            labels=['材料1', '材料2', '材料3', '材料4'],
            patch_artist=True
        )
        ...
        # 设置标题和标签
        plt.title('材料类别平均误差分布', fontsize=16)
        plt.ylabel('绝对误差', fontsize=14)
        plt.xlabel('材料类别', fontsize=14)
        ...
        plt.savefig('材料类别平均误差箱形图.png', dpi=300)
        plt.show()
